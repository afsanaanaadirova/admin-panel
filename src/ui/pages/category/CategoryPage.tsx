...
function CategoryPage() {
  ...
  const deleteHandler = (id: number) => {
    const categoryToDelete = categoriesData?.find((category) => category.id === id);
  
    if (categoryToDelete) {
      const hasProducts = productsData?.some((product) => product.category === categoryToDelete.name);
      hasProducts? dispatch(errorToast(i18n.t("no_remove"))): deleteCategory.mutate(id)
    }
  };
